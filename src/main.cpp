...
const String googleScriptUrl = "https://script.google.com/macros/s/1z_AWbXyup3CxqPMR1lZD-DHHigUYwRHVsDjh3w1EQfk/exec";  // Thay bằng URL của Google Apps Script
...
// Hàm gửi mã QR lên Google Sheets
void sendBarcodeToGoogleSheets(String barcode) {
  if (!client.connect(googleScriptUrl.c_str(), 443)) {
  ...
  // Gửi yêu cầu POST
  client.println("POST /macros/s/1z_AWbXyup3CxqPMR1lZD-DHHigUYwRHVsDjh3w1EQfk/exec HTTP/1.1");
  ...
  while (nanoSerial.available()) {
    ...
    if (line == "<STX>") {
      ...
      if (line.startsWith("BARCODE:")) {
        ...
        sendBarcodeToGoogleSheets(code); // Gửi mã QR vào Google Sheets
